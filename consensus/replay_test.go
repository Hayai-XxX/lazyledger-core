...
func TestSimulateValidatorsChange(t *testing.T) {
	...
	proposal := types.NewProposal(vss[1].Height, round, -1, propBlock.DataAvailabilityHeader)
	p := proposal.ToProto()
	...
	proposal = types.NewProposal(vss[2].Height, round, -1, propBlock.DataAvailabilityHeader)
	p = proposal.ToProto()
	...
	proposal = types.NewProposal(vss[3].Height, round, -1, types.DataAvailabilityHeader{})
	p = proposal.ToProto()
	...
	selfIndex = valIndexFn(0)
	proposal = types.NewProposal(vss[1].Height, round, -1, types.DataAvailabilityHeader{})
	p = proposal.ToProto()
	if err := vss[1].SignProposal(config.ChainID(), p); err != nil {
		t.Fatal("failed to sign bad proposal", err)
	}
