...
func (cs *State) updateToState(state sm.State) {
	...
	cs.ProposalBlockID = types.BlockID{}
	cs.LockedRound = -1
	cs.LockedBlock = nil
	cs.LockedBlockID = types.BlockID{}
	cs.ValidRound = -1
	cs.ValidBlock = nil
	cs.ValidBlockID = types.BlockID{}
...
func (cs *State) defaultDoPrevote(height int64, round int32) {
	...
	if cs.LockedBlock != nil {
		...
		cs.signAddVote(tmproto.PrevoteType, cs.LockedBlock.Hash())
		return
	}
	...
	if cs.ProposalBlock == nil {
		...
		cs.signAddVote(tmproto.PrevoteType, nil)
		return
	}
	...
	cs.signAddVote(tmproto.PrevoteType, cs.ProposalBlock.Hash())
}
...
func (cs *State) enterPrecommit(height int64, round int32) {
	...
	if !ok {
		...
		cs.signAddVote(tmproto.PrecommitType, nil)
		return
	}
	...
	if cs.LockedBlock.HashesTo(blockID.Hash) {
		...
		cs.signAddVote(tmproto.PrecommitType, blockID.Hash)
		return
	}
	...
	if cs.ProposalBlock.HashesTo(blockID.Hash) {
		...
		cs.LockedBlockID = cs.ProposalBlockID
	...
	// In case we have a polka and known proposed BlockID does not match to majority
	if !cs.ProposalBlockID.Equals(blockID) {
		cs.ProposalBlock = nil // remove unaccepted block we might have
		cs.ProposalBlockID = blockID // and set ID of the accepted one
	}
	if err := cs.eventBus.PublishEventUnlock(cs.RoundStateEvent()); err != nil {
		cs.Logger.Error("Error publishing event unlock", "err", err)
	}
...
func (cs *State) enterCommit(height int64, commitRound int32) {
	...
	if cs.LockedBlock.HashesTo(blockID.Hash) {
		...
		cs.ProposalBlockID = cs.LockedBlockID
	}
...
func (cs *State) finalizeCommit(height int64) {
	...
	if cs.blockStore.Height() < block.Height {
		...
		cs.blockStore.SaveBlock(block, &types.PartSet{}, seenCommit)
	} else {
...
func (cs *State) defaultSetProposal(proposal *types.Proposal) error {
	...
	if cs.ProposalBlockID.IsZero() {
		// cs.ProposalBlockID = proposal.BlockID
	}
	cs.Logger.Info("Received proposal", "proposal", proposal)
	return nil
}

func (cs *State) addProposalBlock(msg *BlockMessage, peerID p2p.ID) (added bool, err error) {
	height, round, block := msg.Height, msg.Round, msg.Block

	// Blocks might be reused, so round mismatch is OK
	if cs.Height != height {
		cs.Logger.Debug("Received block part from wrong height",
	...
	if err := cs.eventBus.PublishEventCompleteDAProposal(cs.CompleteDAProposalEvent()); err != nil {
		cs.Logger.Error("Error publishing event complete proposal", "err", err)
	}
	...
	if hasTwoThirds && !blockID.IsZero() && (cs.ValidRound < cs.Round) {
		if cs.ProposalBlock.HashesTo(blockID.Hash) {
			...
			cs.ValidBlockID = cs.ProposalBlockID
		}
